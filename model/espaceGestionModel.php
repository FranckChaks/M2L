...
function displayUncheckedFormation(){
    global $bdd;
    $req = $bdd->prepare("SELECT DISTINCT s.prenom, s.nom, contenu, s.id_s, f.id_f FROM salarie s, participer p, formation f WHERE p.etat = 0 AND estChef = 0 AND s.id_s = p.id_s AND f.id_f = p.id_f AND id_c = ".$_SESSION['id']);
    $req->execute();
    return $req->fetchAll();
}